#!/usr/bin/env python
"""
Provides classes and functionality to help streamline the
process of creating classes whose instances can persist
their state-data in any of several back-end data-stores.
"""

# Adapted from concepts in Brian Allbee's original
# Python-code blog,
...
"""
    ...
    def _set_created(
        ...
        if type(value) not in (datetime, str, float, int):
            raise TypeError(
                '%s.created expects a datetime value, a '
                'numeric value (float or int) that can '
                'be converted to one, or a string value '
                'of the format "%s" that can be parsed '
                'into one, but was passed "%s" (%s)' %
                (
            ...
            # - A numeric value was passed, so create a
            #   new value from it
            try:
                value = datetime.fromtimestamp(value)
            except Exception as error:
                raise ValueError(
                    '%s.created could not create a valid '
                    'datetime object from the value '
                    'provided, "%s" (%s) due to an '
                    'error - %s: %s' %
                    (
            ...
            # - A string value was passed, so create a
            #   new value by parsing it with the standard
            #   format
            try:
                ...
                raise ValueError(
                    '%s.created could not parse a valid '
                    'datetime object using "%s" from the '
                    'value provided, "%s" (%s) due to an '
                    'error - %s: %s' %
                    (
                        self.__class__.__name__,
                        self.__class__._data_time_string,
        ...
        #   we have a well-formed datetime object, so
        #   store it
        self._created = value
    ...
    def _set_is_active(self, value: (bool, int)):
        if value not in (True, False, 1, 0):
            raise ValueError(
                '%s.is_active expects either a boolean '
    ...
    def _set_is_deleted(self, value: (bool, int)):
        if value not in (True, False, 1, 0):
            raise ValueError(
                '%s.is_deleted expects either a boolean '
                'value (True|False) or a direct int-'
                'value equivalent (1|0), but was passed '
                '"%s" (%s)' %
                (
    ...
    def _set_is_dirty(self, value: (bool, int)):
        if value not in (True, False, 1, 0):
            raise ValueError(
                '%s.is_dirty expects either a boolean '
                'value (True|False) or a direct int-'
                'value equivalent (1|0), but was passed '
                '"%s" (%s)' %
                (
    ...
    def _set_is_new(self, value: (bool, int)):
        if value not in (True, False, 1, 0):
            raise ValueError(
                '%s.is_new expects either a boolean '
                'value (True|False) or a direct int-'
                'value equivalent (1|0), but was passed '
                '"%s" (%s)' %
                (
    ...
    def _set_modified(
        ...
        if type(value) not in (datetime, str, float, int):
            raise TypeError(
                '%s.modified expects a datetime value, a '
                'numeric value (float or int) that can '
                'be converted to one, or a string value '
                'of the format "%s" that can be parsed '
                'into one, but was passed "%s" (%s)' %
            ...
            # - A numeric value was passed, so create
            #   a new value from it
            try:
                value = datetime.fromtimestamp(value)
            except Exception as error:
                raise ValueError(
                    '%s.modified could not create a '
                    'valid datetime object from the '
                    'value provided, "%s" (%s) due to '
                    'an error - %s: %s' %
                    (
                        self.__class__.__name__, value,
                        type(value).__name__,
            ...
            try:
                ...
                raise ValueError(
                    '%s.modified could not parse a valid '
                    'datetime object using "%s" from the '
                    'value provided, "%s" (%s) due to an '
                    'error - %s: %s' %
                    (
    ...
    def _set_oid(self, value: (UUID, str)):
        if type(value) not in (UUID, str):
            raise TypeError(
                ...
                'representation of one, but was passed '
                '"%s" (%s)' %
                (
                    self.__class__.__name__,
                    value, type(value).__name__
        ...
        if type(value) == str:
            try:
                ...
                raise ValueError(
                    '%s.oid could not create a valid '
                    'UUID from the provided string "%s" '
                    'because of an error %s: %s' %
                    (
                        self.__class__.__name__, value,
                        error.__class__.__name__, error
                    )
    ...
    created = property(
        ...
        'Gets, sets or deletes the date-time that the '
        'state-data record of the instance was created'
    )
    is_active = property(
        _get_is_active, _set_is_active, _del_is_active,
        'Gets sets or deletes the flag that indicates '
        'whether the instance is considered active/'
        'available'
    )
    is_deleted = property(
        _get_is_deleted, _set_is_deleted, _del_is_deleted,
        'Gets sets or deletes the flag that indicates '
        'whether the instance is considered to be '
        '"deleted," and thus not generally available'
    )
    is_dirty = property(
        _get_is_dirty, _set_is_dirty, _del_is_dirty,
        'Gets sets or deletes the flag that indicates '
        'whether the instance\'s state-data has been '
        'changed such that its record needs to be updated'
    )
    is_new = property(
        _get_is_new, _set_is_new, _del_is_new,
        'Gets sets or deletes the flag that indicates '
        'whether the instance needs to have a state-data '
        'record created'
    )
    modified = property(
        _get_modified, _set_modified, _del_modified,
        'Gets, sets or deletes the date-time that the '
        'state-data record of the instance was last '
        'modified'
    )
    oid = property(
        _get_oid, _set_oid, _del_oid,
        'Gets, sets or deletes the unique identifier of '
        'the instance\'s state-data record in the back-'
        'end data-store'
    )
...
"""
        raise NotImplementedError(
            '%s has not implemented _create, as required '
            'by BaseDataObject'
            % (self.__class__.__name__)
        )
    ...
    def to_data_dict(self) -> (dict,):
        """
Returns a dictionary representation of the instance which
can be used to generate data-store records, or for
criteria-matching with the matches method.
"""
        raise NotImplementedError(
            '%s has not implemented _create, as required '
            'by BaseDataObject'
    ...
    def _update(self) -> None:
        """
Updates an existing state-data record for the instance
in the back-end data-store
"""
        raise NotImplementedError(
            '%s has not implemented _update, as required '
            'by BaseDataObject'
            % (self.__class__.__name__)
        )
    ...
    def matches(self, **criteria) -> (bool,):
        """
Compares the supplied criteria with the state-data values
of the instance, and returns True if all instance
properties specified in the criteria exist and equal the
values supplied.
"""
        # - First, if criteria is empty, we can save some
        #   time and simply return True - If no criteria
        #   are specified, then the object is considered
        #   to match the criteria.
        if not criteria:
            return True
        # - Next, we need to check to see if all the
        #   criteria specified even exist in the instance:
        data_dict = self.to_data_dict()
        data_keys = set(data_dict.keys())
        criteria_keys = set(criteria.keys())
        # - If all criteria_keys exist in data_keys, then
        #   the intersection of the two will equal
        #   criteria_keys. If that's not the case, at
        #   least one key-value won't match (because it
        #   doesn't exist), so return False
        if criteria_keys.intersection(data_keys) != \
                criteria_keys:
            return False
        # - Next, we need to verify that values match
        #   for all specified criteria
        return all(
            [
    ...
    def save(self):
        """
Saves the instance's state-data to the back-end data-store
by creating it if the instance is new, or updating it if
...
"""
        raise NotImplementedError(
            '%s.delete (a class method) has not been '
    ...
    def from_data_dict(cls, data_dict: (dict,)):
        """
Creates and returns an instance of the class whose state-
data has been populate with values from the provided
data_dict
"""
        raise NotImplementedError(
            '%s.from_data_dict (a class method) has not '
            'been implemented, as required by '
            'BaseDataObject' % (cls.__name__)
        )
...
"""
        raise NotImplementedError(
            '%s.get (a class method) has not been '
            'implemented, as required by BaseDataObject'
            % (cls.__name__)
        )
...
"""
        raise NotImplementedError(
            '%s.sort (a class method) has not been '
            'implemented, as required by BaseDataObject'
            % (cls.__name__)
        )
