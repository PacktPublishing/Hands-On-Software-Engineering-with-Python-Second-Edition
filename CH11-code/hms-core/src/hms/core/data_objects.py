...
is_dirty ...... (bool|int, optional, defaults to None)
                A flag indicating that the object's data
...
"""
        ...
        #   criteria_keys.
        #   If that's not the case, at least one key-value
        #   won't match (because it doesn't exist), so
        #   return False
        if criteria_keys.intersection(data_keys) \
                != criteria_keys:
            return False
        # - Next, we need to verify that values match for
