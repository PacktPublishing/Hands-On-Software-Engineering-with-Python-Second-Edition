...
#         return self._property_name
    ...
    def __init__(
        ...
        if not self.__class__._file_store_ready:
            # - The first time the class is used, check
            #   the file-storage directory, and if
            #   everything checks out, then re-set the
            #   flag that controls the checks.
            if not os.path.exists(
                self.__class__._file_store_dir
            ):
                # - If the path-specification exists, try
                #   to assure that the *path* exists, and
                #   create it if it doesn't. If the path
                #   can't be created, then that'll be an
                #   issue later too, so it'll need to be
                #   dealt with.
                try:
                    os.makedirs(
                        self.__class__._file_store_dir
                    ...
                    raise PermissionError(
                        '%s cannot create the JSON data-'
                        'store directory (%s) because '
                        'permission was denied. Please '
                        'check permissions on that '
                        'directory (or its parents, if '
                ...
                try:
                    ...
                    raise PermissionError(
                        '%s cannot write files to the '
                        'JSON data-store directory (%s) '
                ...
                # - If no errors were raised, then re-set
                #   the flag:
        ...
        if self.is_new or self.is_dirty:
            ...
            try:
                ...
                fp = open(
                    ...
                    (
                        self.__class__._file_store_dir,
                        os.sep, self.__class__.__name__,
                        os.sep, self.oid
                    ), 'w'
                )
                # - Write the instance's data-dict to the
                #   file as JSON
                json.dump(
                    self.to_data_dict(), fp, indent=4
                )
                ...
                raise PermissionError(
                    '%s could not save an object to the '
                    'JSON data-store directory (%s) '
                    'because permission was denied. '
                    'Please check permissions on that '
            ...
            # - Any other errors will just surface for
            #   the time being
    ...
    def _load_objects(cls, force_load=False):
        ...
        if cls._loaded_objects == None or force_load:
            ...
            # - Get a list of all the JSON files in the
            #   data-store path
            files = [
            ...
            if files:
                for fname in files:
                    ...
                    try:
                        ...
                        #   data
                        instance = cls.from_data_dict(
                            data_dict
                        )
                        # - Keep track of it by oid in
                        #   the class
                        cls._loaded_objects[
                            instance.oid
                        ] = instance
                    # - If permissions are a problem,
                    #   raise an error with helpful
                    #   information
                    except PermissionError:
                        raise PermissionError(
                    ...
                    #   helpful information
                        ...
                        raise error.__class__(
                            '%s could not load object-'
                            'data from the data-store '
                            'file at %s because the data '
                            'was corrupt or not what was '
                            'expected (%s: %s)' %
                            (
                                cls.__name__, item_file,
    ...
    def delete(cls, *oids):
        ...
        #   file, handling any errors raised in the
        #   process.
        failed_deletions = []
        for oid in oids:
            try:
                # - Try to delete the file first, so that
                #   deletion failures won't leave the
                #   files but remove the in-memory copies
        ...
        if failed_deletions:
            # - Though we *are* raising an error here,
            #   *some* deletions may have succeeded. If
            #   this error-message is displayed, the user
            #   seeing it need only be concerned with the
            #   items that failed, though...
            raise PermissionError(
                '%s.delete could not delete %d object-'
                'data %s because permission was denied. '
                'Please check the permissions on %s and '
                'try again' %
                (
    ...
    def get(cls, *oids, **criteria):
        ...
        if oids:
            ...
            if not criteria:
                results = [
                    o for o
                    in cls._loaded_objects.values()
                    if str(o.oid) in oids
                ]
            # - Otherwise, we *also* need to use matches
            #   to find items that match the criteria
            else:
                results = [
                    o for o
                    in cls._loaded_objects.values()
        ...
        #   criteria
            ...
            return results
        # - If neither were specified, return all items
        #   available:
        else:
