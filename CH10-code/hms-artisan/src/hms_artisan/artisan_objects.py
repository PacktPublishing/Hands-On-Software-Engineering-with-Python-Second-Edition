...
"""
    ...
    def _set_company_name(self, value: str) -> None:
        # - Call the parent method
        result = BaseArtisan._set_company_name(
            self, value
        )
        self._set_is_dirty(True)
        return result
    ...
    def _set_contact_email(self, value: str) -> None:
        # - Call the parent method
        result = BaseArtisan._set_contact_email(
            self, value
        )
        self._set_is_dirty(True)
        return result
    ...
    def _set_contact_name(self, value: str) -> None:
        # - Call the parent method
        result = BaseArtisan._set_contact_name(
            self, value
        )
        self._set_is_dirty(True)
        return result
...
self .............. (Artisan instance, required) The
                    instance to execute against
contact_name ...... (str, required) The name of the
                    primary contact for the Artisan that
                    the instance represents
contact_email ..... (str [email address], required) The
                    email address of the primary contact
                    for the Artisan that the instance
                    represents
address ........... (Address, required) The mailing/
                    shipping address for the Artisan that
                    the instance represents
...
"""
    ...
    def _load_objects(cls, force_load=False):
        return JSONFileDataObject._load_objects(
            cls, force_load
        )
...
"""
    ...
    def _set_building_address(
        ...
        result = Address._set_building_address(
            self, value
        )
        self._set_is_dirty(True)
        return result
    ...
    building_address = property(
        ...
        _set_building_address, _del_building_address,
    ...
    items = property(
        ...
        'Gets the items associated with the order, a '
        'dict of OID keys with quantity values'
    )
    name = property(
        _get_name, _set_name, _del_name,
        'Gets, sets or deletes the name associated with '
        'the order'
    )
    region = property(
        Address._get_region, _set_region, _del_region,
        'Gets, sets or deletes the region (str|None) of '
        'the instance'
    )
...
region ............ (str, optional, defaults to None) The
                    region (state, territory, etc.)
...
"""
        # - Call parent initializers if needed
        Address.__init__(
            self, street_address, city, building_address,
    ...
    def set_item_quantity(
        ...
        if type(oid) not in (UUID, str):
            raise TypeError(
                '%s.set_item_quantity expects a UUID or '
                'string representation of one for its '
                'oid argument, but was passed "%s" (%s)' %
                (
        ...
        if type(oid) == str:
            try:
                ...
                raise ValueError(
                    '%s.set_item_quantity expects a UUID '
                    'or string representation of one for '
                    'its oid argument, but was passed '
                    '"%s" (%s) which could not be '
        ...
        if type(quantity) != int:
            raise TypeError(
                '%s.set_item_quantity expects non-'
                'negative int-value for its quantity '
...
#         return self._property_name
    ...
    def _set_shipping_weight(self, value: (int, float)):
        result = BaseProduct._set_shipping_weight(
            self, value
        )
        self._set_is_dirty(True)
        return result
    ...
    def _set_store_available(self, value: (bool, int)):
        result = BaseProduct._set_store_available(
            self, value
        )
        self._set_is_dirty(True)
        return result
